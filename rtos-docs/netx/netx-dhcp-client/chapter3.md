...
---
# <a name="chapter-3---description-of-azure-rtos-netx-dhcp-client-services"></a>Bölüm 3-Azure RTOS NetX DHCP Istemci hizmetlerinin açıklaması

Bu bölüm, tüm Azure RTOS NetX DHCP hizmetlerinin (aşağıda listelenmiştir) alfabetik sırada bir açıklamasını içerir.
...
- **nx_dhcp_server_address_get**: *DHCP istemcisinin DHCP sunucu adresini al*

- **nx_dhcp_set_interface_index**: *istemci ağ arabirimini belirtin*

- **nx_dhcp_start**: *DHCP işlemeyi Başlat*

- **nx_dhcp_state_change_notify**: *DHCP durum değişikliğinin uygulamasına bildirme*

- **nx_dhcp_stop**: *DHCP işlemesini durdur*

- **nx_dhcp_user_option_retrieve**: *DHCP seçeneğini Al*
...
- **NX_SUCCESS** (0x00) başarılı DHCP oluştur
...
**Iş parçacıkları,** Başlatılmasında
...
Uygulamanın öncelikle nx_ip_interface_attach kullanarak bu arabirimi IP örneğiyle kaydetmesi gerektiğini aklınızda *.*
...
Bir arabirimi DHCP için devre dışı bırakmak için uygulama *nx_dhcp_interface_disable* hizmetini çağırabilir.
...
- **NX_DHCP_NO_RECORDS_AVAILABLE** (0xa7) DHCP için başka bir arabirimin etkinleştirilebilmesi için kullanılabilir kayıt yok

- DHCP için **NX_DHCP_INTERFACE_ALREADY_ENABLED** (0xA3) arabirimi etkinleştirildi
...
- **NX_SUCCESS** (0x00) yayın bayrağını başarıyla güncelleştirildi
...
Bu hizmet, uygulamanın bu hizmet çağrısında daha önce oluşturulmuş bir paket havuzuna bir işaretçi geçirerek DHCP Istemci paket havuzunu oluşturmasına izin verir. Bu özelliği kullanmak için, ana bilgisayar uygulamasının NX_DHCP_CLIENT_USER_CREATE_PACKET_POOL tanımlanmalıdır. Tanımlandığında, *nx_dhcp_create* hizmet istemcinin paket havuzunu oluşturmaz. Uygulamanın, paket havuzu oluştururken *nx_dhcp. h* içinde NX_DHCP_PACKET_PAYLOAD olarak tanımlanan DHCP istemci paket havuzu yükünün varsayılan değerlerini kullanması önerildiğini unutmayın.

### <a name="input-parameters"></a>Giriş Parametreleri

- **dhcp_ptr** DHCP denetim bloğu işaretçisi.  

- **packet_pool_ptr** Önceden oluşturulmuş paket havuzuna yönelik işaretçi
...
- NX_DHCP_INVALID_PAYLOAD (0x9C) yükü çok küçük
...
- **client_ip_address** DHCP sunucusundan istek yapılacak IP adresi

- **skip_discover_message** Doğru ise, DHCP Istemcisi Istek iletisi gönderir  
...
- **Interface_index** IP adresi isteği için arabirim dizini

- **client_ip_address** DHCP sunucusundan istek yapılacak IP adresi

- **skip_discover_message** Doğru ise, DHCP Istemcisi Istek iletisi gönderir; diğer bir deyişle, bulma iletisini gönderir.
...
- **dhcp_ptr** Önceden oluşturulmuş DHCP örneğine yönelik işaretçi
...
- **NX_SUCCESS** (0x00) başarılı bir DHCP sürümü.  
...
Bu hizmet, DHCP için etkinleştirilen tüm arabirimlerde DHCP sunucusundan kiralanmış bir IP adresini reddeder. NX_DHCP_CLIENT_ SEND_ ARP_PROBE tanımlanmışsa, DHCP Istemcisi IP adresinin zaten kullanımda olduğunu algılarsa bir reddetme iletisi gönderir. NetX DHCP Istemcisinde ARP araştırma yapılandırması hakkında daha fazla bilgi için bkz. bölüm içinde **ARP araştırmaları** .
...
- **dhcp_message_type** İleti isteği ( *nx_dhcp. h* içinde tanımlanmıştır)
...
- **Interface_index** İletinin gönderileceği arabirimin dizini  
...
Bu hizmet DHCP istemci kaydında bulunan DHCP için etkinleştirilen ilk arabirimdeki DHCP Istemci DHCP sunucusu IP adresini alır. Çağıran bu hizmeti yalnızca DHCP Istemcisi DHCP sunucusu tarafından atanan bir IP adresine bağlandıktan sonra kullanabilir. Ana bilgisayar uygulaması, IP adresinin ayarlandığını doğrulamak için *nx_ip_status_check* hizmetini kullanabilir veya *NX_DHCP_STATE_CHANGE_NOTIFY* kullanabilir ve DHCP istemci durumunun NX_DHCP_STATE_BOUND olduğunu sorgulayabilir. Durum değişikliği geri arama işlevini ayarlama hakkında daha fazla ayrıntı için bkz. *nx_dhcp_state_change_notify* .
...
Bu hizmetin DHCP Istemcisini yalnızca bir arabirimde çalıştırmayı sağlayan uygulamalar için tasarlandığını unutmayın. Birden çok arabirimde DHCP çalıştırmak için daha fazla ayrıntı için *nx_dhcp_interface_enable* bakın.
