...
Azure RTOS NetX DHCP sunucusu, konumundaki ortak kaynak kodu deposundan elde edilebilir [https://github.com/azure-rtos/netx/](https://github.com/azure-rtos/netx/) . Paket, aşağıdaki gibi, bu belgeyi içeren iki kaynak dosya ve bir PDF dosyası içerir:

- **nx_dhcp_server. h**: NETX DHCP sunucusu için üst bilgi dosyası
- **nx_dhcp_server. c**: NETX DHCP sunucusu Için c kaynak dosyası
- **nx_dhcp_server.pdf**: NETX DHCP sunucusunun PDF açıklaması
- **demo_netx_dhcp_server. c**: NETX DHCP sunucusu gösterimi

## <a name="dhcp-installation"></a>DHCP yüklemesi

NetX DHCP sunucusunu kullanabilmeniz için, daha önce bahsedilen dağıtımın tamamı NetX 'in yüklü olduğu dizine kopyalanmalıdır. Örneğin, "*\threadx\arm7\green*" dizininde NETX yüklüyse, *nx_dhcp_server. h* ve *nx_dhpc_server. c* dosyaları bu dizine kopyalanmalıdır.

## <a name="using-netx-dhcp-server"></a>NetX DHCP sunucusu kullanma

NetX DHCP sunucusu kullanmak kolaydır. Temel olarak, uygulama kodu sırasıyla ThreadX ve NetX kullanmak için *tx_api. h* ve *nx_api. h* dahil *nx_dhcp_server.* h içermelidir. *Nx_dhcp_server. h* dahil olduğunda, uygulama kodu daha sonra bu KıLAVUZDA belirtilen DHCP işlev çağrılarını yapabilir. Uygulama, yapı işlemine *nx_dhcp_server. c* de içermelidir. Bu dosya, diğer uygulama dosyalarıyla aynı şekilde derlenmelidir ve nesne formu, uygulamanın dosyalarıyla birlikte bağlanmalıdır. NetX DHCP sunucusunu kullanma hakkında daha fazla bilgi için, NETX [DHCP sunucusunun](#constraints-of-the-netx-dhcp-server) [NETX dhcpserver](#requirements-of-the-netx-dhcp-server) ve kısıtlamalarına yönelik aşağıdaki bölümlerde yer alan gereksinimlere bakın.

> [!NOTE]
> DHCP, NetX UDP hizmetlerini kullandığından, DHCP kullanılmadan önce *nx_udp_enable* çağrısıyla UDP 'nin etkinleştirilmesi gerekir.

## <a name="requirements-of-the-netx-dhcp-server"></a>NetX DHCP sunucusunun gereksinimleri

NetX DHCP sunucusu, tanınmış DHCP bağlantı noktası 67 ' e atanmış bir UDP yuva bağlantı noktası gerektirir. Uygulama, DHCP sunucusunu oluşturmak için en az 548 bayt artı IP, UDP ve Ethernet üst bilgileri (4 baytlık hizalamayla toplam 44 bayt) ile birlikte paket yüküne sahip bir paket havuzu oluşturması gerekir.
...
NetX DHCP sunucusu, Istemciye geri ileten TEKLIF ve DHCPACK iletilerindeki parametre Istek seçeneği (55) listesine yüklenecek bir seçenek kümesiyle yapılandırılmalıdır. Bu seçenekler, Istemci ağı için ağ açısından kritik yapılandırma verilerini içermeli ve varsayılan olarak yönlendirici IP adresi, alt ağ maskesi ve DNS sunucusu olarak tanımlanmıştır. Seçenek listesi, boşlukla ayrılmış bir liste ve Kullanıcı tarafından yapılandırılabilir NX_DHCP_DEFAULT_SERVER_OPTION_LIST tanımlı bir listesidir. Bu listede belirtilen seçeneklerin sayısı aynı zamanda Kullanıcı tanımlı NX_DHCP_DEFAULT_OPTION_LIST_SIZE eşit olmalıdır.

## <a name="constraints-of-the-netx-dhcp-server"></a>NetX DHCP sunucusunun kısıtlamaları

### <a name="dhcp-messages"></a>DHCP Iletileri

NetX DHCP sunucusu, IP adresini Istemciye vermeden önce ağ üzerinde başka bir yerde bir IP adresi atanmamış olduğunu doğrulamaz. Birden çok DHCP sunucusu varsa, bu durum gerçekten olabilir. *RFC 2131 ' e kadar, IP adresinin ağda benzersiz olduğunu doğrulama* (örneğin, adrese ping) olduğu için istemcinin sorumluluğundadır. Değilse, sunucu, veritabanını Istemciden güncelleştirmek için IP adresine sahip bir reddetme iletisi almalıdır.

NetX DHCP sunucusu FORCE_RENEW iletileri vermiyor. IP adresi kiralamasını yenilemek için DHCP Istemcisine kadar olur. Ancak, DHCP sunucusu, tüm atanan IP adreslerinde kalan süreyi veritabanında izler. Bir IP adresi kirası sona erdiğinde, bu IP adresi kullanılabilir IP adresleri havuzuna döndürülür. Bu nedenle, IP adresi kiralamasını etkin bir şekilde yenilemek/yeniden bağlamak için Istemciye sahip olur.
...
NetX DHCP sunucusu, istemci verilerini Istemci kayıt tablosuna DHCP isteği parametreleri dahil kaydeder. Bu tablo geçici olmayan bellekte depolanmaz. bu nedenle, DHCP sunucusu konağının, yeniden başlatmalar arasında bilgilerin kaydedilmemesi gerekir.

NetX DHCP sunucusu IP adresi kiralama verilerini bir IP adresi tablosuna kaydeder. Bu tablo geçici olmayan bellekte depolanmaz. bu nedenle, DHCP sunucusu konağının, yeniden başlatmalar arasında bilgilerin kaydedilmemesi gerekir.

### <a name="relay-agents"></a>Geçiş aracıları

NetX DHCP sunucusu, ağ DHCP isteklerini desteklemediğinden, ' Relay Agent ' alanı için sıfır bir IP adresi ile yapılandırılır.

## <a name="small-example-system"></a>Küçük örnek sistem

Aşağıda görüntülenen Şekil 1,1 ' de NetX DHCP sunucusunu kullanmanın ne kadar kolay olduğunu gösteren bir örnek. Bu örnekte, *nx_dhcp_server. h* DHCP içerme dosyası 5. satırda getirilir. DHCP sunucu iş parçacığı yığın boyutu, IP iş parçacığı yığın boyutu ve test iş parçacığı yığın boyutu, 7-13 satırlarında tanımlanmıştır.
...
Şekil 1,1 örnek NetX DHCP sunucusu uygulaması

## <a name="configuration-options"></a>Yapılandırma seçenekleri

NetX DHCP sunucusu oluşturmaya yönelik birkaç yapılandırma seçeneği vardır. Aşağıdaki listede her biri ayrıntılı açıklanmıştır:  
  
- **NX_DISABLE_ERROR_CHECKING**: Bu seçenek temel DHCP hata denetimini kaldırır. Genellikle uygulamanın hataları ayıklandıktan sonra kullanılır.  
...
- **NX_DHCP_QUEUE_DEPTH** Kuyruğu temizlemeye başlamadan önce DHCP sunucusu yuvasının sakladığı paketlerin sayısını belirtir. Varsayılan değer 5 ' tir.
