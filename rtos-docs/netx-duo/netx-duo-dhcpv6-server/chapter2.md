---
title: Bölüm 2-Azure RTOS NetX Duo DHCPv6 sunucusunu yükleme ve kullanma
description: Bu bölüm, NetX Duo DHCPv6 sunucusunun yüklenmesiyle, kurulumuyla ve kullanımıyla ilgili çeşitli sorunların açıklamasını içerir.
author: philmea
ms.author: philmea
ms.date: 06/08/2020
ms.topic: article
...
ms.locfileid: "104826038"
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a><span data-ttu-id="828d7-103">Bölüm 2-Azure RTOS NetX Duo DHCPv6 sunucusunu yükleme ve kullanma</span><span class="sxs-lookup"><span data-stu-id="828d7-103">Chapter 2 - Installation and use of Azure RTOS NetX Duo DHCPv6 server</span></span>

<span data-ttu-id="828d7-104">Bu bölüm, NetX Duo DHCPv6 sunucusunun yüklenmesiyle, kurulumuyla ve kullanımıyla ilgili çeşitli sorunların açıklamasını içerir.</span><span class="sxs-lookup"><span data-stu-id="828d7-104">This chapter contains a description of various issues related to installation, setup, and usage of the NetX Duo DHCPv6 Server.</span></span>

## <a name="product-distribution"></a><span data-ttu-id="828d7-105">Ürün dağıtımı</span><span class="sxs-lookup"><span data-stu-id="828d7-105">Product Distribution</span></span>

<span data-ttu-id="828d7-106">Azure RTOS NetX Duo, konumundaki ortak kaynak kodu deposundan elde edilebilir [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/) .</span><span class="sxs-lookup"><span data-stu-id="828d7-106">Azure RTOS NetX Duo can be obtained from our public source code repository at [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/).</span></span>

<span data-ttu-id="828d7-107">**nxd_dhcpv6_server. h** NetX DuoDHCPv6Server üst bilgi dosyası</span><span class="sxs-lookup"><span data-stu-id="828d7-107">**nxd_dhcpv6_server.h** NetX DuoDHCPv6Server header file</span></span>

<span data-ttu-id="828d7-108">**nxd_dhcpv6_server. c** NetX DuoDHCPv6Server kaynak dosyası</span><span class="sxs-lookup"><span data-stu-id="828d7-108">**nxd_dhcpv6_server.c** NetX DuoDHCPv6Server source file</span></span>

<span data-ttu-id="828d7-109">**demo_netxduo_dhcpv6. c** NetX Duo DHCPv6 sunucusu tanıtım dosyası</span><span class="sxs-lookup"><span data-stu-id="828d7-109">**demo_netxduo_dhcpv6.c** NetX Duo DHCPv6 Server demo file</span></span>

<span data-ttu-id="828d7-110">**nxd_dhcpv6_server.pdf** NetX Duo DHCPv6Server Kullanıcı Kılavuzu</span><span class="sxs-lookup"><span data-stu-id="828d7-110">**nxd_dhcpv6_server.pdf** NetX Duo DHCPv6Server User Guide</span></span>

## <a name="netx-duo-dhcpv6-server-installation"></a><span data-ttu-id="828d7-111">NetX Duo DHCPv6 sunucusu yüklemesi</span><span class="sxs-lookup"><span data-stu-id="828d7-111">NetX Duo DHCPv6 Server Installation</span></span>

<span data-ttu-id="828d7-112">NetX Duo DHCPv6Server API 'sini kullanmak için, daha önce bahsedilen tüm dağıtımın, NetX Duo 'un yüklü olduğu dizine kopyalanması gerekir.</span><span class="sxs-lookup"><span data-stu-id="828d7-112">In order to use the NetX Duo DHCPv6Server API, the entire distribution mentioned previously should be copied to the same directory where NetX Duo is installed.</span></span> <span data-ttu-id="828d7-113">Örneğin, "*\threadx\arm7\green*" dizininde NETX Duo yüklüyse, *nxd_dhcpv6_server. h* ve *nx_dhpcv6_server. c* dosyaları bu dizine kopyalanmalıdır.</span><span class="sxs-lookup"><span data-stu-id="828d7-113">For example, if NetX Duo is installed in the directory “*\threadx\arm7\green*” then the *nxd_dhcpv6_server.h* and *nx_dhpcv6_server.c* files should be copied into this directory.</span></span>

## <a name="using-netx-duo-dhcpv6-server"></a><span data-ttu-id="828d7-114">NetX Duo DHCPv6 sunucusunu kullanma</span><span class="sxs-lookup"><span data-stu-id="828d7-114">Using NetX Duo DHCPv6 Server</span></span>

<span data-ttu-id="828d7-115">NetX Duo DHCPv6Server API 'sini kullanmak kolaydır.</span><span class="sxs-lookup"><span data-stu-id="828d7-115">Using the NetX Duo DHCPv6Server API is easy.</span></span> <span data-ttu-id="828d7-116">Temel olarak, uygulama kodu sırasıyla ThreadX ve NetX Duo kullanmak için *tx_api. h* ve *nx_api. h* dahil *nx_dhcpv6-Server. h* içermelidir.</span><span class="sxs-lookup"><span data-stu-id="828d7-116">Basically, the application code must include *nx_dhcpv6-server.h* after it includes *tx_api.h* and *nx_api.h*, in order to use ThreadX and NetX Duo, respectively.</span></span> <span data-ttu-id="828d7-117">Uygulama, yapı işlemine *nxd_dhcpv6_server. c* de içermelidir.</span><span class="sxs-lookup"><span data-stu-id="828d7-117">The application must also include *nxd_dhcpv6_server.c* in the build process.</span></span> <span data-ttu-id="828d7-118">Bu dosya, diğer uygulama dosyalarıyla aynı şekilde derlenmelidir ve nesne formu, uygulamanın dosyalarıyla birlikte bağlanmalıdır.</span><span class="sxs-lookup"><span data-stu-id="828d7-118">This file must be compiled in the same manner as other application files and its object form must be linked along with the files of the application.</span></span> <span data-ttu-id="828d7-119">Bu, NetX Duo DHCPv6 sunucusunu kullanmak için gereklidir.</span><span class="sxs-lookup"><span data-stu-id="828d7-119">This is all that is required to use NetX Duo DHCPv6 Server.</span></span>

<span data-ttu-id="828d7-120">DHCPv6is bu yana IPv6 protokolünü temel alan, *nxd_ipv6_enable* kullanılarak IP örneğinde IPv6 'nın etkin olması gerektiğini unutmayın.</span><span class="sxs-lookup"><span data-stu-id="828d7-120">Note that since DHCPv6is based on the IPv6 protocol, IPv6 must be enabled on the IP instance using *nxd_ipv6_enable*.</span></span> <span data-ttu-id="828d7-121">NetX Duo UDP ve ICMPv6 hizmetleri de kullanılır.</span><span class="sxs-lookup"><span data-stu-id="828d7-121">NetX Duo UDP and ICMPv6 services are also utilized.</span></span> <span data-ttu-id="828d7-122">UDP, NetX Duo DHCPv6 sunucusu iş parçacığı görevi başlatılmadan önce *nxd_icmp_enable* çağırarak *nx_udp_enable* ve ICMPv6is etkin olarak etkinleştirilir.</span><span class="sxs-lookup"><span data-stu-id="828d7-122">UDP is enabled by calling *nx_udp_enable* and ICMPv6is enabled by calling *nxd_icmp_enable* prior to starting the NetX Duo DHCPv6 Server thread task.</span></span>

## <a name="small-example-system"></a><span data-ttu-id="828d7-123">Küçük örnek sistem</span><span class="sxs-lookup"><span data-stu-id="828d7-123">Small Example System</span></span>

<span data-ttu-id="828d7-124">NetX Duo DHCPv6 sunucusunu kullanmanın ne kadar kolay olduğunu gösteren bir örnek, bir DHCPv6 Istemcisi ve sanal "RAM" sürücüsü üzerinden çalışan sunucu kullanarak aşağıdaki küçük örnekte açıklanmıştır.</span><span class="sxs-lookup"><span data-stu-id="828d7-124">An example of how easy it is to use the NetX Duo DHCPv6 Server is described in the small example below using a DHCPv6 Client and Server running over a virtual “RAM” driver.</span></span> <span data-ttu-id="828d7-125">Bu tanıtımda NetX Duo ortamını kullanan tek bir bağlı olan ana bilgisayar varsayılmaktadır.</span><span class="sxs-lookup"><span data-stu-id="828d7-125">This demo assumes a single homed host using the NetX Duo environment.</span></span>

<span data-ttu-id="828d7-126">*tx_application_define* , hem istemci hem de sunucu için DHCPV6 iletisi, bir iş parçacığı ve bir IP örneği göndermek üzere paket havuzu oluşturur ve 116-157 numaralı satırlarda hem istemci hem de sunucu IP GÖREVLERI için UDP (DHCP 'nin UDP üzerinden çalışır), ıPV6, ICMP ve ICMPv6 'yi sunar.</span><span class="sxs-lookup"><span data-stu-id="828d7-126">*tx_application_define* creates packet pool for sending DHCPv6 message, a thread and an IP instance for both the Client and Server, and enables UDP (DHCP runs over UDP), IPv6, ICMP and ICMPv6 for both Client and Server IP tasks in lines 116-157.</span></span>

<span data-ttu-id="828d7-127">DHCPv6 sunucusu 456. satırda oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="828d7-127">The DHCPv6 Server is created in line 456.</span></span> <span data-ttu-id="828d7-128">İsteğe bağlı adres reddetme veya seçenek isteği işleyicilerini tanımlamaz.</span><span class="sxs-lookup"><span data-stu-id="828d7-128">It does not define the optional address decline or option request handlers.</span></span> <span data-ttu-id="828d7-129">Sunucu iş parçacığı girişi işlevinde, sunucu IP 'si, 435-453. satırlardaki bir bağlantı yerel adres hizmetleriyle ayarlanır.</span><span class="sxs-lookup"><span data-stu-id="828d7-129">In the Server thread entry function, the Server IP is set up with a link local address services in lines 435-453.</span></span>

<span data-ttu-id="828d7-130">DHCPv6 sunucusunu başlatmadan önce, ana bilgisayar uygulaması 498 satırında bir sunucu DUıD oluşturur ve yerel ağ DNS sunucusunu 483. satırda ayarlar.</span><span class="sxs-lookup"><span data-stu-id="828d7-130">Before starting the DHCPv6 Server, the host application creates a Server DUID in line 498 and sets the local network DNS server on line 483.</span></span> <span data-ttu-id="828d7-131">Daha sonra 521 satırlarında atanabilir IP adresleri tablosu oluşturur.</span><span class="sxs-lookup"><span data-stu-id="828d7-131">It then creates a table of assignable IP addresses in lines 521.</span></span> <span data-ttu-id="828d7-132">Sunucu tablolarını depolama ve bellekten alma hakkında daha fazla bilgi için bkz. Ek D 'de **Gelişmiş örnek sistem** .</span><span class="sxs-lookup"><span data-stu-id="828d7-132">See the **Advanced Example System** in Appendix D for how to store and retrieve Server tables from memory.</span></span>

<span data-ttu-id="828d7-133">Ardından, DHCPv6 sunucusu 530. satırda başlatılmaya hazırlanın.</span><span class="sxs-lookup"><span data-stu-id="828d7-133">Then the DHCPv6 Server is ready to start on line 530.</span></span>

<span data-ttu-id="828d7-134">NetX Duo DHCPv6 Istemcisini oluşturma ve çalıştırma hakkında daha fazla bilgi için bkz. DHCPv6 sunucusuyla dağıtılan *nxd_dhcpv6_client.pdf* dosyası.</span><span class="sxs-lookup"><span data-stu-id="828d7-134">For details on creating and running the NetX Duo DHCPv6 Client see the *nxd_dhcpv6_client.pdf* file distributed on with the DHCPv6 Server.</span></span>
...
<span data-ttu-id="828d7-135">**Şekil 6. NetX Duo DHCPv6 sunucusu örneği**</span><span class="sxs-lookup"><span data-stu-id="828d7-135">**Figure 6. Example of the NetX Duo DHCPv6 Server**</span></span>