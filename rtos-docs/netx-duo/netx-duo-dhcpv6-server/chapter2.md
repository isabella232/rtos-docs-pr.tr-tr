...
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a>Bölüm 2-Azure RTOS NetX Duo DHCPv6 sunucusunu yükleme ve kullanma

Bu bölüm, NetX Duo DHCPv6 sunucusunun yüklenmesiyle, kurulumuyla ve kullanımıyla ilgili çeşitli sorunların açıklamasını içerir.
...
**nxd_dhcpv6_server. h** NetX DuoDHCPv6Server üst bilgi dosyası

**nxd_dhcpv6_server. c** NetX DuoDHCPv6Server kaynak dosyası

**demo_netxduo_dhcpv6. c** NetX Duo DHCPv6 sunucusu tanıtım dosyası

**nxd_dhcpv6_server.pdf** NetX Duo DHCPv6Server Kullanıcı Kılavuzu

## <a name="netx-duo-dhcpv6-server-installation"></a>NetX Duo DHCPv6 sunucusu yüklemesi

NetX Duo DHCPv6Server API 'sini kullanmak için, daha önce bahsedilen tüm dağıtımın, NetX Duo 'un yüklü olduğu dizine kopyalanması gerekir. Örneğin, "*\threadx\arm7\green*" dizininde NETX Duo yüklüyse, *nxd_dhcpv6_server. h* ve *nx_dhpcv6_server. c* dosyaları bu dizine kopyalanmalıdır.

## <a name="using-netx-duo-dhcpv6-server"></a>NetX Duo DHCPv6 sunucusunu kullanma

NetX Duo DHCPv6Server API 'sini kullanmak kolaydır. Temel olarak, uygulama kodu sırasıyla ThreadX ve NetX Duo kullanmak için *tx_api. h* ve *nx_api. h* dahil *nx_dhcpv6-Server. h* içermelidir. Uygulama, yapı işlemine *nxd_dhcpv6_server. c* de içermelidir. Bu dosya, diğer uygulama dosyalarıyla aynı şekilde derlenmelidir ve nesne formu, uygulamanın dosyalarıyla birlikte bağlanmalıdır. Bu, NetX Duo DHCPv6 sunucusunu kullanmak için gereklidir.

DHCPv6is bu yana IPv6 protokolünü temel alan, *nxd_ipv6_enable* kullanılarak IP örneğinde IPv6 'nın etkin olması gerektiğini unutmayın. NetX Duo UDP ve ICMPv6 hizmetleri de kullanılır. UDP, NetX Duo DHCPv6 sunucusu iş parçacığı görevi başlatılmadan önce *nxd_icmp_enable* çağırarak *nx_udp_enable* ve ICMPv6is etkin olarak etkinleştirilir.

## <a name="small-example-system"></a>Küçük örnek sistem

NetX Duo DHCPv6 sunucusunu kullanmanın ne kadar kolay olduğunu gösteren bir örnek, bir DHCPv6 Istemcisi ve sanal "RAM" sürücüsü üzerinden çalışan sunucu kullanarak aşağıdaki küçük örnekte açıklanmıştır. Bu tanıtımda NetX Duo ortamını kullanan tek bir bağlı olan ana bilgisayar varsayılmaktadır.

*tx_application_define* , hem istemci hem de sunucu için DHCPV6 iletisi, bir iş parçacığı ve bir IP örneği göndermek üzere paket havuzu oluşturur ve 116-157 numaralı satırlarda hem istemci hem de sunucu IP GÖREVLERI için UDP (DHCP 'nin UDP üzerinden çalışır), ıPV6, ICMP ve ICMPv6 'yi sunar.

DHCPv6 sunucusu 456. satırda oluşturulur. İsteğe bağlı adres reddetme veya seçenek isteği işleyicilerini tanımlamaz. Sunucu iş parçacığı girişi işlevinde, sunucu IP 'si, 435-453. satırlardaki bir bağlantı yerel adres hizmetleriyle ayarlanır.

DHCPv6 sunucusunu başlatmadan önce, ana bilgisayar uygulaması 498 satırında bir sunucu DUıD oluşturur ve yerel ağ DNS sunucusunu 483. satırda ayarlar. Daha sonra 521 satırlarında atanabilir IP adresleri tablosu oluşturur. Sunucu tablolarını depolama ve bellekten alma hakkında daha fazla bilgi için bkz. Ek D 'de **Gelişmiş örnek sistem** .

Ardından, DHCPv6 sunucusu 530. satırda başlatılmaya hazırlanın.

NetX Duo DHCPv6 Istemcisini oluşturma ve çalıştırma hakkında daha fazla bilgi için bkz. DHCPv6 sunucusuyla dağıtılan *nxd_dhcpv6_client.pdf* dosyası.
...
**Şekil 6. NetX Duo DHCPv6 sunucusu örneği**